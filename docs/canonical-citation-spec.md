...
2. **Court parenthetical.** Otherwise, the court identified from the citation's court parenthetical (`(11th Cir. 2019)` -> courts-db ID `ca11`), validated against courts-db, gives `us-` + ID. The standard federal appellate forms are recognizable per se: `1st` through `11th` (including the Bluebook ordinals `2d`/`3d`), `D.C.`, and `Fed.`, followed by `Cir.`, map to `ca1`-`ca11`, `cadc`, `cafc` even where an underlying library misses an ordinal variant (measured: eyecite 2.7.6 resolves `3rd Cir.` but not the Bluebook-standard `3d Cir.`; the reference implementation compensates with an exact whitelist, which is not a guess).
