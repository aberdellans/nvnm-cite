...
## Status
- Current phase: Phase 4 CORE COMPLETE (tasks 4.1–4.6 done; 4.3 dropped). Receipts anchor + verify round-tripped LIVE on testnet (registry `inveniam--mata-v-avianca` + Mata ECF24 receipt; verify confirms + catches a one-byte tamper); the unified `nvnm-cite` CLI (check/anchor/verify/stats + delegated sync/rebuild-index/reconcile/load/update) is wired; opt-in query telemetry records by-citation counts only. Phase 3 COMPLETE + tagged phase-3-done. 487 tests green. NOT yet tagged phase-4-done: the webapp receipt-flow update + clerk-legible registry view remain (they live in Phase 4.5).
- Last completed: 4.5 unified CLI + 4.6 telemetry (`verifier/telemetry.py`); tests in tests/test_cli.py, tests/test_telemetry.py. Anchoring stays gated behind `--anchor` + an explicit in-chat OK.
- Next up: Phase 4.5 (web app) — wire the webapp receipt flow to the locked v1 (drop kya_id + draft schema + compaction ladder; per-firm-per-case create; Verify takes registry+file), enable + disclose telemetry, the clerk-legible registry view, and the copy rewrites (Check/About/Inspect/FAQ) + StatusService fast-fail. Then P5 (MCP), P6 (demo). Tranche 2 stays gated by choice.
