...
- Next up: 4.6 RPC query telemetry (by citation, decoupled from document hash + client identity); update the webapp receipt flow to the locked v1 (drop kya_id + draft schema + compaction ladder; per-firm-per-case create; Verify takes registry+file) — this last is best done within the Phase 4.5 web pass (copy/UX + clerk-legible registry view). Tranche 2 stays gated by choice.
