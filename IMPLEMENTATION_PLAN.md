...
## Status
- Current phase: Phase 4 IN PROGRESS. Receipts anchor + verify (tasks 4.1/4.2/4.4) DONE and round-tripped LIVE on testnet (2026-06-15): registry `inveniam--mata-v-avianca` created + the Mata ECF24 receipt anchored; `nvnm-cite verify` confirms it and catches a one-byte tamper. Phase 3 COMPLETE + tagged phase-3-done (shared live-read verifier core + `nvnm-cite check`). 479 tests green.
- Last completed: `receipts/anchor.py` (4.2) + `receipts/verify.py` (4.4) + CLI `anchor`/`verify` wiring (tests in tests/test_receipts_flow.py, tests/test_cli.py). Anchoring is gated behind `--anchor` + an explicit in-chat OK; this session's two live writes were approved.
- Next up (no design forks left): 4.6 RPC query telemetry (by citation, decoupled from document hash + client identity); finish 4.5 CLI (`sync` / `rebuild-index` / `reconcile` / `stats` / `load` over the existing indexer/reconcile/bulk_load modules); update the webapp receipt flow to the locked v1 (drop kya_id + draft schema + compaction ladder; per-firm-per-case create; Verify takes registry+file). Then Phase 4.5 (web copy/UX). Tranche 2 stays gated by choice.
...
- [ ] 4.5 `cli.py`: `nvnm-cite check | anchor | verify | sync | rebuild-index | reconcile | stats | load`. `stats` (and Phase 5's registry_stats/coverage) derive figures from chain_index.sqlite with its sync head stated, never from the precompile's countTotal (unreliable per 0.7 (g)). IN PROGRESS: `check`/`anchor`/`verify` implemented + live-validated (`src/nvnm_cite/cli.py`); `sync`/`rebuild-index`/`reconcile`/`stats`/`load` remain (fold in the existing indexer/reconcile/bulk_load module CLIs).
