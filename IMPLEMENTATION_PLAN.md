...
## Status
- Current phase: Phase 2 nearly COMPLETE (2.1-2.6 done; only 2.7 daily updater remains). Tranche-1 load finished 2026-06-13: 260,763 records on testnet (us-scotus=737, us-ca11=738), reconcile CLEAN (0 missing / 0 extra / 0 drift vs a full chain re-read).
- Last completed: task 2.6 - load + indexer sync + reconcile CLEAN. Indexer hardened mid-task (transport retry in EvmRpc, opt-in) after a sync crashed on IncompleteRead; 426 tests green.
- Next up: 2.7 `loader/update.py` (daily incremental append via REST, cron-shaped, --dry-run). That finishes Phase 2 (then tag phase-2-done). Tranche 2 (SCOTUS memoranda) stays gated; budget is no longer a constraint (bulk top-up).
