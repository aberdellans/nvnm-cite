...
## Status
- Current phase: Phase 4 IN PROGRESS. Task 4.1 LOCKED (2026-06-15): the minimal non-enumerating receipt v1 + filer-chosen `<firm>--<case>` registry naming in `src/nvnm_cite/receipts/schema.py` (pure, no chain); docs/record-schema.md §2/§4 re-locked. Phase 3 COMPLETE + tagged phase-3-done (shared live-read verifier core + `nvnm-cite check`). 468 tests green.
- Last completed: task 4.1 (receipt + registry-naming lock; tests in tests/test_receipts.py). The REMAINING Phase 4 tasks involve chain WRITES and are gated on an explicit per-transaction OK.
- Next up: 4.2 `receipts/anchor.py` (create the per-firm-per-case registry + anchor a receipt — WRITES), 4.4 `receipts/verify.py` ((registry + file) → confirm + recompute + one-byte-tamper catch), 4.5 CLI `anchor`/`verify` wiring (+ remaining subcommands), 4.6 RPC query telemetry, and update the webapp receipt flow to the locked v1. Then Phase 4.5 (remaining web copy/UX). Tranche 2 stays gated by choice.
